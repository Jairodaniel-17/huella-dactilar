...
def thining1(R):
    return (
        parathining1(R)
        or parathining1(np.rot90(R, k=1))
        or parathining1(np.rot90(R, k=2))
        or parathining1(np.rot90(R, k=3))
    )
...
def thining2(R):
    return (
        parathining2(R)
        or parathining2(np.rot90(R, k=1))
        or parathining2(np.rot90(R, k=2))
        or parathining2(np.rot90(R, k=3))
    )
...
def pruning1(R):
    return (
        parapruning1(R)
        or parapruning1(np.rot90(R, k=1))
        or parapruning1(np.rot90(R, k=2))
        or parapruning1(np.rot90(R, k=3))
    )
...
def pruning2(R):
    return (
        parapruning2(R)
        or parapruning2(np.rot90(R, k=1))
        or parapruning2(np.rot90(R, k=2))
        or parapruning2(np.rot90(R, k=3))
    )
...
def adelgazar(Img):
    for i in range(1, m - 1):
        for j in range(1, n - 1):
            if Img[i, j]:
                R = Img[i - 1 : i + 2, j - 1 : j + 2].astype(
                    int
                )  # obtengo un retazo de 3x3
                Img[i, j] = int(
                    not (
                        thining1(R)
                        or thining2(R)
    ...
    for i in range(1, m - 1):
        for j in range(1, n - 1):
            ...
            if B[i, j]:
                ...
                cv2.circle(imgRGB, (j, i), 5, (0, 0, 255), 1)
    print("Terminaciones: " + str(terminacion))
    print("Bifurcaciones : " + str(bifurcacion))
    imgRGB = cv2.resize(imgRGB, None, fx=2.5, fy=2.5, interpolation=cv2.INTER_AREA)
